...
class Pi0FASTStateConfig(_model.BaseModelConfig):
    dtype: str = "bfloat16"
    ...
    def model_type(self) -> _model.ModelType:
        return _model.ModelType.PI0_FAST_STATE

    @override
    def create(self, rng: at.KeyArrayLike) -> "Pi0FASTState":
        return Pi0FASTState(self, rngs=nnx.Rngs(rng))

    @override
    def inputs_spec(self, *, batch_size: int = 1) -> tuple[_model.ObservationWithState, _model.Actions]:
        image_spec = jax.ShapeDtypeStruct([batch_size, *_model.IMAGE_RESOLUTION, 3], jnp.float32)
        image_mask_spec = jax.ShapeDtypeStruct([batch_size], jnp.bool_)

        with at.disable_typechecking():
            observation_spec = _model.ObservationWithState(
                images={
                ...
                state={
                    "robot_state":jax.ShapeDtypeStruct([batch_size, self.action_dim], jnp.float32),
                    "obj_pose":jax.ShapeDtypeStruct([batch_size, 6], jnp.float32),
                    "ee_pose":jax.ShapeDtypeStruct([batch_size, 6], jnp.float32),
                },
...
class Pi0FASTState(_model.BaseModel):
    def __init__(self, config: Pi0FASTStateConfig, rngs: nnx.Rngs):
    ...
    @at.typecheck
    def embed_inputs(
        self, obs: _model.ObservationWithState
    ...
    def sample_obj_states(
        self,
        rng: at.KeyArrayLike,
        observation: _model.ObservationWithState,
        *,
        ...
        # sample output object state tokens
        # prepare decoding -- final logit decodes the first token
        last_logit = prefix_logits[:, -1:]
        obj_state_tokens = jnp.zeros((last_logit.shape[0], max_decoding_steps))
                
        def step(carry):
            last_logit, obj_state_tokens, cache, _, step = carry
            ...
            obj_state_tokens = put_along_last_axis(obj_state_tokens, jnp.broadcast_to(step, (token.shape[0], 1)), token)
            ...
            return last_logit, obj_state_tokens, kv_cache, all_eos, step + 1
        ...
        _, obj_state_tokens, _, _, _ = jax.lax.while_loop(cond, step, (last_logit, obj_state_tokens, kv_cache, False, 0))
        return obj_state_tokens
