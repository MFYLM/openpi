...
# Added for state dict input with maniskill
@dataclasses.dataclass(frozen=True)
class ManiskillStateInputs(transforms.DataTransformFn):
    """Transform Maniskill data format to model input format.
    ...
    # Determines which model will be used
    model_type: _model.ModelType = _model.ModelType.PI0_FAST_STATE
    ...
    def __call__(self, data: dict) -> dict:
        # Get state and pad to model action dimension
        # state = transforms.pad_to_dim(data["observation/state"], self.action_dim)
        ee_pose_pad = transforms.pad_to_dim(data["ee_pose"], self.action_dim)
